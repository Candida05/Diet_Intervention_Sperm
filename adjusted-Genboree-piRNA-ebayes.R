setwd("~/DI-manuscript-rev/DE-piRNA-analysis")
...
metrics <- read.delim("All-sample-factors-ed.txt", row.names=1, header=T, na.strings=c("Missing","N/A","NA","Not tested")) %>% 
...
counts <- read.delim("Genboree-piRNA-expression-profile-orig.txt", row.names="PID", header = T) # Raw piRNA expression profile for all 34 samples
colnames(counts)
...
plot(table(rs))
keep <- which(rs >= 8)
raw <- raw[keep, , keep.lib.sizes=FALSE]  # 13911 -> 8429
dim(raw)
rm(rs, keep)
...
write.table(tt.adjusted, file="DE-Genboree-piRNAs.txt", row.names = TRUE, col.names = TRUE, quote = FALSE, sep = "\t") #main result

tt <- topTable(efit, coef="intervention_over_control", n=Inf) %>% 
  rownames_to_column("pirs") %>% 
mutate(LOD=-log10(P.Value), sig=sign(logFC)*( abs(logFC) > log2(1.5) & LOD >= -log10(0.01)), supersig=sign(logFC)*( abs(logFC) > log2(1.5) & LOD >= -log10(0.001)) ) %>% # higher confidence 0.01
select(pirs, logFC, P.Value, LOD, sig, supersig)


library(ggrepel)

tiff("volcano-DE-Genboree-piRNAs.tiff", height = 20, width = 20, units = "cm", res=300) #main result

ggplot(tt, aes(x=logFC, y=LOD)) + 
  geom_point(data=subset(tt, sig==0), size=1) +
  geom_point(data=subset(tt, sig!=0), col="salmon", size=1) + 
  geom_point(data=subset(tt, supersig!=0), col="salmon", size=1) +
  theme_classic(base_size = 18) + 
  xlab("log2 fold change") + ylab("-log10 p-value") + 
  xlim(-10, 10) + ylim(0,5)+
  geom_text_repel(data=subset(tt, supersig!=0), aes(label=pirs, fontface=2), 
