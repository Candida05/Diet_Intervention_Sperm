...
counts <- read.delim("MV1-miRNA-CPM-TMM.txt", header = T) # 17 samples
colnames(counts)
length(counts)
...
for(i in 53:length(data))
{
  ...
  if (rescor$p.value < 0.011)# get 2 at 0.011 cut-off 
  {
    ...
    p <- (ggplot(data, aes(x=Age,  y=data[,i])) + geom_point(colour="olivedrab3", size =3 ) + geom_smooth(method="lm", colour="black")) + labs(x="Age", y=paste(colnames(data)[i],"(CPM)")) + theme_classic() + theme(axis.text = element_text(color = "black", size = 14), axis.title = element_text(color = "black", size = 16))
      print(p+(stat_cor(method = "spearman", label.x.npc= "left", label.y.npc = "top", size=4)))   
      dev.off()
  }
}

write.table(outputa, file="mirs-age-corr1811.txt", row.names = TRUE, col.names = NA, quote=FALSE, sep = "\t")
...
for(i in 53:length(data))
{
  ...
  if (rescor$p.value < 0.01)# get 2 at 0.01 cut-off
  {
   print (paste(colnames(data)[i], " cor:", rescor$estimate, " p=value:", rescor$p.value))
   tiff(paste(colnames(data)[i],"BMI.tiff"), height = 20, width = 20, units = "cm", res=300)
   p <- (ggplot(data, aes(x=BMI, y=data[,i])) + geom_point(colour="olivedrab3", size =3 ) + geom_smooth(method="lm", colour="black")) + labs(x="BMI", y=paste(colnames(data)[i],"(CPM)")) + theme_classic() + theme(axis.text = element_text(color = "black", size = 14), axis.title = element_text(color = "black", size = 16))
   print(p+(stat_cor(method = "spearman", label.x.npc= "left", label.y.npc = "top", size=4)))   
   dev.off()
  }
...
for(i in 53:length(data))
{
  ...
  if (rescor$p.value < 0.02)# get 2 at 0.02 cut-off 
  {
    print (paste(colnames(data)[i], " cor:", rescor$estimate, " p=value:", rescor$p.value))
    tiff(paste(colnames(data)[i],"Sp-mot.tiff"), height = 20, width = 20, units = "cm", res=300)
    p <- (ggplot(data, aes(x=Percentage.of.sperm.motile...., y=data[,i])) + geom_point(colour="olivedrab3", size =3 ) + geom_smooth(method="lm", colour="black")) + labs(x="Percentage of sperm motile", y=paste(colnames(data)[i],"(CPM)") ) + theme_classic() + theme(axis.text = element_text(color = "black", size = 14), axis.title = element_text(color = "black", size = 16))
    print(p+(stat_cor(method = "spearman", label.x.npc= "left", label.y.npc = "top", size=4)))   
    dev.off()
  }
